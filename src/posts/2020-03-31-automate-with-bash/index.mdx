...
tags: ['productivity', 'bash', 'terminal', 'script']
...
Curl a `URL` and check if it responds with a 200 range status.
The complicated bit is just cutting the status code from the `curl` response header.
